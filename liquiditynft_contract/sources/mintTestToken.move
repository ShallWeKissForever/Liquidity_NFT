module launchpad::launchpad2 {
    ...
    public entry fun create_fa(
        ...
        let fa_owner_obj_constructor_ref = &object::create_object(@launchpad);
        ...
        let registry = borrow_global_mut<Registry>(@launchpad);
    ...
    public fun get_registry(): vector<Object<Metadata>> acquires Registry {
        let registry = borrow_global<Registry>(@launchpad);
        registry.fa_objects
    }
    ...
    fun is_admin(config: &Config, sender: address): bool {
        if (sender == config.admin_addr) {
            ...
            if (object::is_object(@launchpad)) {
                let obj = object::address_to_object<ObjectCore>(@launchpad);
    ...
    #[test(aptos_framework = @0x1, sender = @launchpad)]
    fun test_happy_path(
